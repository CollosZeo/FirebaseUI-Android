...
import static com.firebase.ui.auth.AuthUI.AUTH_TYPE_ACTIVITY;
import static com.firebase.ui.auth.AuthUI.AUTH_TYPE_DIALOG;
...
public class AuthMethodPickerActivity
        ...
        implements IDPProvider.IDPCallback, View.OnClickListener, CompoundButton.OnCheckedChangeListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        int pickerType = flowParams.authMethodPickerType;
        switch (pickerType) {
            case AUTH_TYPE_ACTIVITY:
                setContentView(R.layout.auth_method_picker_layout);
                break;
            case AUTH_TYPE_DIALOG:
                setContentView(R.layout.auth_method_picker_layout_dialog);
                if (flowParams.smartLockEnabled) {
                    View view = findViewById(R.id.smart_lock_holder);
                    if (view != null) {
                        view.setVisibility(View.VISIBLE);
                    }
                    CompoundButton smartLock = (CompoundButton) findViewById(R.id.use_smart_lock);
                    if (smartLock != null) {
                        smartLock.setOnCheckedChangeListener(this);
                    }
                }
                break;
        ...
        populateIdpList(flowParams.providerInfo);

        int logoId = flowParams.logoId;
    ...
    protected void onDestroy() {
        ...
        @AuthUI.AuthPickerType
        int pickerType = flowParams.authMethodPickerType;
        if (flowParams.smartLockEnabled && pickerType == AUTH_TYPE_DIALOG) {
            CompoundButton smartLock = (CompoundButton) findViewById(R.id.use_smart_lock);
            if (smartLock != null) {
                smartLock.setOnCheckedChangeListener(null);
            }
        }
    }
